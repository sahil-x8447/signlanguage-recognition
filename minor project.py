...
def cameracap(image, fingers_statuses, count, draw=True, display=True):
    '''
    This function will determine the gesture of the left and right hand in the image.
    Args:
        image:            The image of the hands on which the hand gesture recognition is required to be performed.
...
def register():
    ...
    password_entry.pack()
    Label(register_screen, text="").pack()
    Button(register_screen, text="Register", width=10, height=1, bg="Coral", command = register_user).pack()
...
def delete_login_success():
    ...
    frame=Frame(width=1200, height=75, bg='red').place(x=0, y=0)

    tk.Label(frame,text="Sign language detection and hand gesture recognition", font="normal 20 bold", fg="black",bg='red').pack(pady=20)
...
# button3.pack(side=tk.BOTTOM)
    ...
    canvas = Canvas(parent, width = 500, height = 500,bg="blue",highlightbackground = "blue")  
    canvas.pack()  
    img = ImageTk.PhotoImage(Image.open("hand gesture.jpg"))  
...
def main_account_screen():
    ...
    Frame(width=800, height=75, bg='red').place(x=0, y=0)
    
    Label(text="𝓦𝓔𝓛𝓒𝓞𝓜𝓔", font=('COPPERPLATE GOTHIC BOLD', 25, 'bold'), fg='white', bg="red").pack()
    #Label(text="").pack()
    Button(text="𝓛𝓞𝓖𝓘𝓝",bg='Coral', height="2", width="30", command = login).place(x=150, y=100)
    #Label(text="").pack()
    Button(text="𝕽𝖊𝖌𝖎𝖘𝖙𝖊𝖗",bg='Coral', height="2", width="30", command=register).place(x=150, y=190)
